...
module.exports = async function NuxtMarkdown (moduleOptions) {
  ...
  const options = {
    ...
    contentClassName: 'nuxt-markdown',
    grayMatter: {},
    markdownIt: {
      html: true
    },
  ...
  for (const collectionOptions of options.collections) {
    ...
    const files = glob.sync(pattern, { cwd: path.join(this.options.srcDir, collection.directory) })

    for (const file of files) {
      // TODO: Allow configuration of gray-matter
      const markdown = matter.read(path.join(this.options.srcDir, collection.directory, file), options.grayMatter)
