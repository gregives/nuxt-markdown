...
module.exports = async function NuxtMarkdown(moduleOptions) {
  ...
  for (const collection of options.collections) {
    ...
    directories[collection.name] = `(f) => import(\`~${path.join(path.sep, collection.directory).split(path.sep).join('/')}/\${f}.md\`)`
  ...
  this.addPlugin({
    ...
    options: {
      ...
      imports: JSON.stringify(directories).replace(/"(\(\w*\) => import\([^"]*\))"/g, '$1')
    }
  })
}
