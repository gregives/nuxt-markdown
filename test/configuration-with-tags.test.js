...
describe('Nuxt Markdown with configuration and tags', () => {
  let nuxt

  beforeAll(async () => {
    ({ nuxt } = await setup(loadConfig(__dirname, '../../examples/configuration-with-tags')))
  }, 60000)
  ...
  it('exposes all Markdown files in the blog collection', async () => {
    const { document } = new JSDOM(await get('/')).window
    expect(document.querySelector('ol').children).toHaveLength(4)
  ...
  it('loads tags of all blog posts using serverTransform', async () => {
    const { document } = new JSDOM(await get('/tags/')).window
    expect(document.querySelector('ol').children).toHaveLength(6)
  })
})
